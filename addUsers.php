...
if (mb_strlen($login) < 3 || mb_strlen($login) > 100) {
  echo"<script>swal(\"Недопустимая длина логина!\", \"Логин содержит от 3 до 100 символов\", \"error\");</script>";
    ...
    exit();
} elseif (mb_strlen($pass) < 10 || mb_strlen($pass) > 32) {
    echo"<script>swal(\"Недопустимая длина пароля!\", \"Пароль содержит от 10 до 32 символов\", \"error\");</script>";
    exit();
}elseif (mb_strlen($role) < 3 || mb_strlen($role) > 5) {
    echo"<script>swal(\"Недопустимая длина роли!\", \"Роль содержит от 3 до 5 символов\", \"error\");</script>";
    exit();
}
