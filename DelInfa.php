...
<form action="AddInfa.php" method="post">
  <div class="row">
    <div class="col-sm-4">
      <input type="text" id="AddName"name="AddGorod" class="form-control" placeholder="Введите имя">
    </div>
    <div class="col-sm-4">
      <input type="text" id="AddCity"name="AddDos" class="form-control" placeholder="Введите город">
    </div>
    <div class="col-sm-4">
      <input type="text" id="AddSex"name="AddTypeDos" class="form-control" placeholder="Введите пол">
    </div>
    <div class="col-sm-4">
      <input type="text" id="AddAge"name="AddCafe" class="form-control" placeholder="Введите возраст">
    </div>
  </div>
...
$result = $mysql->query("SELECT * FROM `population` WHERE `id`='$id'");
$user = $result->fetch_assoc();
if(count($user)==0){
  echo"<script>swal(\"Такой информации нет!\", \"Неверный номер строки\", \"error\");</script>";
}else{
  $mysql->query("DELETE FROM population WHERE id = '$id'");
  $mysql->close();
  echo"<script>swal(\"Успешно!\", \"Информация успешно удалена\", \"success\");</script>";
  exit("<meta http-equiv='refresh' content='3; url= /editInfa.php'>");
}
