...
$city = filter_var(trim($_POST['AddCity']), FILTER_SANITIZE_STRING);
$sex = filter_var(trim($_POST['AddSex']), FILTER_SANITIZE_STRING);
$age = filter_var(trim($_POST['AddAge']), FILTER_SANITIZE_STRING);
...
if (strlen($name) < 1 || strlen($name) > 50) {
    ...
    exit();
} elseif (strlen($city) < 1 || strlen($city) > 50) {
    echo"<script>swal(\"Недопустимая длина города!\", \"Город содержит от 1 до 50 символов\", \"error\");</script>";
    exit();
} elseif(! $is_sex_valide){
...
elseif(! is_numeric($age)){
  echo"<script>swal(\"Ошибка!\", \"Неверно заполнено поле возраст\", \"error\");</script>";
  exit();
}
